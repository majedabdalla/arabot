...
ASK_GENDER, ASK_COUNTRY, PROFILE_MENU = range(3)

# All 22 Arab League member states (+ a catch-all), used as the only
# geography step in this Arab-focused bot — no continent/region step.
COUNTRIES = [
    '🇸🇦 السعودية', '🇦🇪 الإمارات', '🇪🇬 مصر', '🇯🇴 الأردن',
    '🇱🇧 لبنان', '🇸🇾 سوريا', '🇮🇶 العراق', '🇰🇼 الكويت',
    '🇶🇦 قطر', '🇧🇭 البحرين', '🇴🇲 عُمان', '🇾🇪 اليمن',
    '🇵🇸 فلسطين', '🇱🇾 ليبيا', '🇹🇳 تونس', '🇩🇿 الجزائر',
    '🇲🇦 المغرب', '🇸🇩 السودان', '🇸🇴 الصومال', '🇩🇯 جيبوتي',
    '🇰🇲 جزر القمر', '🇲🇷 موريتانيا', '🌍 دولة عربية أخرى'
]

def _country_kb(prefix):
    """Lay countries out two-per-row instead of one long column."""
    rows = []
    for i in range(0, len(COUNTRIES), 2):
        pair = COUNTRIES[i:i + 2]
        rows.append([InlineKeyboardButton(c, callback_data=f"{prefix}{c}") for c in pair])
    return rows
...
async def unified_profile_entry(update: Update, context):
    ...
    if existing:
        ...
        if notify_admin and admin_group:
            # Build the updated user data for the mention (use new values)
            ...
            try:
                await context.bot.send_message(
                    chat_id=admin_group, text=msg, parse_mode='HTML'
                )
    ...
    if not existing:
        ...
        if not existing.get('gender') or not existing.get('country'):
            kb = InlineKeyboardMarkup([
                [InlineKeyboardButton(locale.get('gender_male', 'Male'), callback_data='gender_male'),
                 InlineKeyboardButton(locale.get('gender_female', 'Female'), callback_data='gender_female')]
            ])
...
async def gender_cb(update: Update, context):
    ...
    kb = InlineKeyboardMarkup(_country_kb("country_"))
...
async def country_cb(update: Update, context):
    ...
    if admin_group:
        try:
            await context.bot.send_message(
                chat_id=admin_group, text=profile_text, parse_mode='HTML'
            )
